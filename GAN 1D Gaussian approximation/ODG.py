...
# First we create the “real” data distribution
class DataDistribution(object):
    def __init__(self):
        self.mean = 4 # mean 4
        self.deviation = 0.5 # standard deviation 0.5

    def sample(self, N):
        samples = np.random.normal(self.mean, self.deviation, N)
        samples.sort()
        return samples

# We also define the generator input noise distribution
...
# In this case we found that it was important to make sure that the discriminator is more powerful than the generator, 
# as otherwise it did not have sufficient capacity to learn to be able to distinguish accurately 
# between generated and real samples. So we made it a deeper neural network, with a larger number of dimensions. 
# It uses tanh nonlinearities in all layers except the final one, which is a sigmoid (the output of which we can interpret as a probability).
...
# We create optimizers for each network using the plain GradientDescentOptimizer in TensorFlow with exponential learning rate decay. 
# We should also note that finding good optimization parameters here did require some tuning.
...
def parse_args():
    ...
    parser.add_argument('--num-steps', type=int, default=5000, help='the number of training steps to take')
    parser.add_argument('--hidden-size', type=int, default=4, help='MLP hidden size')
    parser.add_argument('--batch-size', type=int, default=8, help='the batch size')
    parser.add_argument('--minibatch', action='store_true', help='use minibatch discrimination')
    parser.add_argument('--log-every', type=int, default=10, help='print loss after this many steps')
    parser.add_argument('--anim-path', type=str, default=None, help='path to the output animation file')
    parser.add_argument('--anim-every', type=int, default=1, help='save every Nth frame for animation')
